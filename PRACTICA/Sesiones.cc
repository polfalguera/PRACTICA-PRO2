...
void Sesiones::nueva_sesion(const string& s) 
{
    Sesion sesion;
    sesion.leer_sesion();
...
void Sesiones::modificar_enviables_sesion(const string& s, Problemas& enviables, Problemas& resueltos) const 
{
    map<string,Sesion>::const_iterator it = cjt_sesiones.find(s);
    (*it).second.modificar_enviables_s(enviables,resueltos);
}

void Sesiones::modificar_envibales_envio_s(const string& p, const string& s, Problemas& enviables, Problemas& resueltos) const
{
    map<string,Sesion>::const_iterator it = cjt_sesiones.find(s);
...
string Sesiones::problema_iesimo_sesion(const string& s, int i) const 
{
    map<string,Sesion>::const_iterator it = cjt_sesiones.find(s);
