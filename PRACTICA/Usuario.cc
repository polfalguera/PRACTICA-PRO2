...
void Usuario::actualizar_enviables(const string& p, const string& s, const Problema& problema, Curso& curso, Sesiones& cs ) {
    enviables.eliminar_enviable(p);

    curso.modificar_enviables_s(p,s,enviables,resueltos,cs);    
}
